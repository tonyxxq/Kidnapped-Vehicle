...
int main() {
  ...
  double delta_t = 0.1;  // 两次测量的时间差（秒）
  double sensor_range = 50;  // 传感器测量的范围[m]

  double sigma_pos[3] = { 0.3, 0.3, 0.01 };  // GPS测量的误差范围[x [m], y [m], theta [rad]]
  double sigma_landmark[2] = { 0.3, 0.3 };  // 地表测量误差范围 [x [m], y [m]]
        ...
        if (length && length > 2 && data[0] == '4' && data[1] == '2'){
          ...
          if (s != "") {
            ...
            if (event == "telemetry") {
              ...
              // 判断是否已经初始化滤波器，否:进行初始化 是：预测下一个状态
              if (!pf.initialized()) {
